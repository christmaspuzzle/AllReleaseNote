...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchReleases(forceRefresh = false) {
        try {
            ...
            const savedLang = localStorage.getItem('appLang') || 'en';
            let url = forceRefresh ? '/api/releases/refresh' : '/api/releases';
            if (savedLang === 'ko') {
                url += (url.includes('?') ? '&' : '?') + 'lang=ko';
            }
            ...
            items.forEach(item => {
                ...
                // Override for Gemini Enterprise mobile app as requested
                if (cleanDescription.includes('mobile app in general availability for Google Identity users')) {
                    cleanTitle = 'Gemini Enterprise mobile app: General availability (GA) for Google Identity users';
                }
                ...
                let releaseDate = new Date(pubDate);
                
                // Extract actual release date from description HTML if available
                const releaseDateMatch = descriptionRaw.match(/Release date:.*?<\/strong>\s*([A-Za-z]+ \d{1,2}, \d{4})/i);
                if (releaseDateMatch) {
